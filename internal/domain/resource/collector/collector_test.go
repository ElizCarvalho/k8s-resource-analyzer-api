...
type testCase struct {
	name     string
	testFunc func(*K8sMimirCollector) (interface{}, error)
...
func TestK8sMimirCollector(t *testing.T) {
	...
	tests := []testCase{
		{
			...
			testFunc: func(c *K8sMimirCollector) (interface{}, error) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result, err := tc.testFunc(collector)
...
func TestK8sMimirCollector_GetDeploymentMetrics(t *testing.T) {
	collector := NewK8sMimirCollector(&MockK8sClient{}, &MockMimirClient{})

	metrics, err := collector.GetDeploymentMetrics(context.Background(), "default", "test-deployment")
...
func TestK8sMimirCollector_GetDeploymentConfig(t *testing.T) {
	...
	config, err := collector.GetDeploymentConfig(context.Background(), "default", "test-deployment")
...
func TestK8sMimirCollector_Query(t *testing.T) {
	...
	result, err := collector.Query(context.Background(), "test_query")
...
func TestK8sMimirCollector_QueryRange(t *testing.T) {
	...
	now := time.Now()
	result, err := collector.QueryRange(context.Background(), "test_query", now, now.Add(time.Hour), time.Minute)
